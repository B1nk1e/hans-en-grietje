...
const loadAcceptedScripts = () => {
	const cookieTypesToLoad = JSON.parse(getCookie("hans-grietje_consent"));
...
const initCookieBar = () => {
	...
	if (!cookiebar && getCookie("hans-grietje_consent").length > 0) {
		loadAcceptedScripts();
		return;
	}
	...
	if (!getCookie("hans-grietje_consent").length > 0) {
	...
	const acceptCookies = () => {
		...
		setCookie("hans-grietje_consent", JSON.stringify(acceptedCookieTypes), 30);
