...
@if (!isset($_COOKIE['hans-grietje_consent']) && $title)
    <section class="cookie-consent">
            ...
            @foreach ($cookieTypes as $key => $type)
                <label class="custom-control custom-control--checkbox">
                    <input type="checkbox" class="custom-control__input"
                        {{ $type['slug'] == 'necessary' ? 'disabled' : '' }} checked name="checkbox"
                        value="{{ $type['slug'] }}">
        ...
        <div class="cookie-consent__buttons">
            <button class="btn btn--rose cookie-consent__btn">
                {{ $cookieBtn ? $cookieBtn : __('I agree', 'sage') }}
            </button>

            @if ($cookieInfo['url'])
                <a href="{{ $cookieInfo['url'] }}"
                    target="{{ $cookieInfo['target'] ? $cookieInfo['target'] : '_self' }}" class="btn btn--rose btn--ol"
